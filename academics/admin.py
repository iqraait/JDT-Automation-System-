...
class FeeStructureAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            path('api/options/', self.admin_site.admin_view(self.get_filtered_options), name='academics_feestructure_api_options'),
        ]
        return custom_urls + urls

    def get_filtered_options(self, request):
        ...
        if institute_id:
            try:
                ...
                data['courses'] = [{'id': obj.id, 'name': str(obj)} for obj in c_qs]

                cls_qs = Class.objects.filter(institute_id=inst_id)
                if course_id:
                    try:
                        cls_qs = cls_qs.filter(course_id=int(course_id))
                ...
                if class_id:
                    ...
                    try:
                        cy_qs = cy_qs.filter(class_obj__course_id=int(course_id))
                    except (ValueError, TypeError):
                        pass
                cy_qs = cy_qs.order_by('name')
            ...
            try:
                c_id = int(course_id)
                cls_qs = Class.objects.filter(course_id=c_id).order_by('name')
                data['classes'] = [{'id': obj.id, 'name': str(obj), 'course_id': obj.course_id} for obj in cls_qs]

                cy_qs = ClassYear.objects.filter(class_obj__course_id=c_id, is_active=True).order_by('name')
                data['class_years'] = [{'id': obj.id, 'name': str(obj), 'class_obj_id': obj.class_obj_id} for obj in cy_qs]
            except (ValueError, TypeError):
                pass
        elif class_id:
            try:
                cl_id = int(class_id)
                cy_qs = ClassYear.objects.filter(class_obj_id=cl_id, is_active=True).order_by('name')
                data['class_years'] = [{'id': obj.id, 'name': str(obj), 'class_obj_id': obj.class_obj_id} for obj in cy_qs]
            except (ValueError, TypeError):
                pass
